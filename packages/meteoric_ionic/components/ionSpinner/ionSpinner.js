...
Template.ionSpinner.helpers({
  ...
  icon() {
    const iconName = `spinner-${this.icon || 'ios'}`;
    return iconName;
  },
});
...
Template.ionSpinner.onRendered(function () {
  ...
  const iconElement = this.firstNode;
  this.data = this.data || {};
  const iconName = this.data.icon || 'ios';
  ...
  function init() {
    const spinnerName = iconName;
    const $element = $(iconElement);
    const container = iconElement;
